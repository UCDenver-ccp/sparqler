(ns matsu_sparql
  "Matsu SPARQL que DSL functions"
  (:refer-clojure :exclude [filter concat group-by max min count])
  (:require [clojure.walk :refer [postwalk-replace]]
            [matsu_compiler :refer [compile-query encode]]
            [matsu_core :refer [empty-query]]))
...
(defn !# [comment] (raw (str "\n# " comment " .\n")))


(defn subselect [& more]
  {:tag "SELECT " :content (vec more) :sep " "})

(defmacro subquery
  [args & body]
  `(group (subselect ~@args) (group ~@body)))

(defn prefix [q more]
  (assoc q :prefix {:tag "" :content more :bounds "" :sep ""}))
